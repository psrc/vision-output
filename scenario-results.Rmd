...
roadway_folder <- "L:/vision2050/soundcast/wild_bill/2050/outputs/network"
transit_folder <- "L:/vision2050/soundcast/wild_bill/2050/outputs/transit"
...
```{r create_boarding_maps, include = FALSE}

# Open Transit Results by Node
w_fname <- paste0(transit_folder,"/transit_summary.xlsx")
transit_outputs <- read_excel(w_fname,sheet = "Transit Stop Activity")
setDT(transit_outputs)

# Open Network File to get Lat and Lon for Nodes
w_fname <- paste0(roadway_folder,"/network_shape.csv")
node_outputs <- read.csv(w_fname)
setDT(node_outputs) 
node_outputs <- node_outputs[shape_local_index == 0]
keep_cols <- c("i","lon","lat")
node_outputs <- node_outputs[,..keep_cols]
setkey(node_outputs, i)
node_list <- node_outputs[, j=list(mean(lon, na.rm = TRUE),mean(lat, na.rm = TRUE)),by = i]
setnames(node_list,c("i","lon","lat"))

# Combine Node Ouput with lon,lat for spatial analysis
keep_cols <- c("inode","total_boardings","transfers")
transit_outputs <- transit_outputs[,..keep_cols]
setnames(transit_outputs,c("i","boardings","transfers"))
transit_outputs <- merge(transit_outputs,node_list,by="i")
changeCols <- c("i","boardings","transfers")
transit_outputs <- transit_outputs[,(changeCols):= lapply(.SD, as.integer), .SDcols = changeCols]

# Join Nodes to TAZ's
interim <- transit_outputs
coordinates(interim) <- ~lon+lat
crs(interim) <- CRS("+proj=longlat +datum=WGS84")
taz.shape <- readOGR(dsn='c:/coding/vision-output/inputs/shapefiles',layer='taz2010_wgs1984')
crs(taz.shape) <- CRS("+proj=longlat +datum=WGS84")
taz_id <- over(interim,taz.shape[,'TAZ'])
transit_outputs$TAZ <- taz_id$TAZ
setkey(transit_outputs, TAZ)
summary_boardings <- transit_outputs[, j=list(sum(boardings, na.rm = TRUE),sum(transfers, na.rm = TRUE)),by = TAZ]
setnames(summary_boardings,c("TAZ","boardings","transfers"))

# Merge Boardings by TAZ with the TAZ Shapefile
boardings_to_map <- sp::merge(taz.shape, summary_boardings, by = "TAZ")
boardings_to_map@data[is.na(boardings_to_map@data)] <- 0

rng <- range(boardings_to_map$boardings)
max_bin <- max(abs(rng))
...
labels <- paste0("<b>","TAZ: ", "</b>",boardings_to_map$TAZ,
                 "<b> <br>","Boardings: ", "</b>", prettyNum(round(boardings_to_map$boardings, 0), big.mark = ",")) %>% lapply(htmltools::HTML)

pal <- colorBin("Blues", domain = boardings_to_map$boardings, bins = bins)

working_map <- leaflet(data = boardings_to_map) %>% 
  addProviderTiles(providers$CartoDB.Positron) %>%
  addPolygons(fillColor = pal(boardings_to_map$boardings),
              weight = 1,
              opacity = 1,
              color = "white",
              dashArray = "3",
...
working_map <- working_map %>%
  addLegend("bottomright", pal=pal, values = boardings_to_map$boardings,
            title = "Daily Boardings",
            opacity = 1)

```

```{r plot_boardings, echo = FALSE}
working_map
```
...
# AM Peak Hour
create_network_shapefiles(roadway_folder,output_folder,'7to8')
...
#PM Peak Hour
create_network_shapefiles(roadway_folder,output_folder,'17to18')
...
## County {.tabset .tabset.fade .tabset-pills}
xxx
```{r per_cap_cnty_summaries, include=FALSE}
out <- NULL
...
for (measures in travel_measures) {
  summary_attributes <- c('Vehicle Metrics by Year')
  summary_columns <- c("Today","2050","Delta","% Change")
  w_att <- list('Average VMT per Resident','Average VHT per Resident', 'Average Auto Delay per Resident')
  w_geo <- list(measures[[2]])
  w_grp <- "Total"
  output_type <- "decimal"
  out <- c(out, knitr::knit_child("transportation-metrics-child.Rmd"))
...
## Regional Geography {.tabset .tabset.fade .tabset-pills}
xxx
```{r per_cap_rgeo_summaries, include=FALSE}
out <- NULL
...
## Equity Geography {.tabset .tabset.fade .tabset-pills}
xxx
```{r per_cap_eq_summaries, include=FALSE}
out <- NULL
...
## County {.tabset .tabset.fade .tabset-pills}
xxx
```{r job_acc_cnty_summaries, include=FALSE}
out <- NULL

travel_measures <- list(list("King","King County"),list("Kitsap","Kitsap County"), list("Pierce","Pierce County"), list("Snohomish","Snohomish County"), list("Region","Region"))

for (measures in travel_measures) {
  summary_attributes <- c('Metrics by Year')
  summary_columns <- c("Today","2050","Delta","% Change")
  w_att <- list('Jobs within 1-mile Walk Trip','Jobs within 3-mile Bike Trip', 'Jobs within 45-min Transit Trip')
  w_geo <- list(measures[[2]])
  w_grp <- "Total"
  output_type <- "integer"
  out <- c(out, knitr::knit_child("transportation-metrics-child.Rmd"))
...
## Equity Geography {.tabset .tabset.fade .tabset-pills}
xxx
```{r job_acc_eq_summaries, include=FALSE}
out <- NULL

travel_measures <- list(list("< 50% people of color","Under 50% People of Color"),list("> 50% people of color"," Over 50% People of Color"), list("< 50% low income","Under 50% People with Low Income"), list("> 50% low income"," Over 50% People with Low Income"), list("Region","Region"))
for (measures in travel_measures) {
  summary_attributes <- c('Metrics by Year')
  summary_columns <- c("Today","2050","Delta","% Change")
  w_att <- list('Jobs within 1-mile Walk Trip','Jobs within 3-mile Bike Trip', 'Jobs within 45-min Transit Trip')
