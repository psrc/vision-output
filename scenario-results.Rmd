...
wrk_scenario <- "quick_kick"

# Lists for Analysis
alternatives <- list(c(wrk_scenario, "LRT", "PPA 65/75"))
...
hct_types <- list(100,101:105,106,100:106)
hct_names <- c("Rest of Region","High Capacity Transit","Local Transit","Total")
hct_order <- c("High Capacity Transit","Local Transit","Rest of Region","Total")
...
**Scenario: `r toString(alternatives[[1]][[3]])` (aka `r toString(alternatives[[1]][[1]])`)**
...
for (w_years in analysis_years) {
  w_tbl <- NULL
  w_tbl <- create_table_from_input(paste0("inputs/",wrk_scenario),paste0("parcel__dataset_table__households_jobs__",w_years),"tab")
...
for (i in my_list) {
  summary_geo <- "Region"
  header_names <- parcel_lookups[[i]][[6]]
  wrk_id <- parcel_lookups[[i]][[1]]
  wrk_nms <- parcel_lookups[[i]][[4]]
  wrk_ord <- parcel_lookups[[i]][[5]]
...
# Share of Growth by Area Type {.tabset .tabset.fade .tabset-pills}
By `r toString(analysis_years[[2]])`, more than 100 light rail and streetcar stations are planned in the region. In addition, every county has plans for bus rapid transit and several are planning for commuter rail and passenger-only ferry service expansion. The planning for this integrated regional transit network has been on-going and the latest blueprint for it was adopted in the Regional Transportation Plan (RTP) update in 2018. The charts below illustrate the share of population growth for the `r toString(scenario_names)` scenario summarized in this document.


```{r growth-pie-charts, echo = FALSE, include = FALSE, results = 'asis'}
out <- NULL
...
for (i in my_list) {
  ...
  wrk_nms <- parcel_lookups[[i]][[4]]
  wrk_ord <- parcel_lookups[[i]][[5]]
  out <- c(out, knitr::knit_child("pie-charts-by-type-child.Rmd"))
}
```
...
# Share of Growth by Transit Type {.tabset .tabset.fade .tabset-pills}
...

```{r county-growth-pie-charts, echo = FALSE, include = FALSE, results = 'asis'}
out <- NULL
...
# Capacity by Transit Type {.tabset .tabset.fade .tabset-pills}
...
